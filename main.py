...
def get_net(net_name, weight_path=None):
    ...
    if net_name in ['vgg', 'vgg16']:
        ...
        net = ReIDNet(class_num=751,testing=False,usinghpa=True,hpamodel='densenet',hpaepoch='last',featuresize=48)
    # 加载指定路径的权重参数
...
def main(args):
    ...
    # if args.layer_name is None else
...
if __name__ == '__main__':
    ...
    parser.add_argument('--network', type=str, default='multifeature',
                        help='ImageNet classification network')
    parser.add_argument('--image-path', type=str, default='./examples/5.jpg',
                        help='input image path')
    parser.add_argument('--weight-path', type=str, default='E:\workspace_python\Grad-CAM.pytorch-master/net_49.pth',help='weight path of the model')
    # parser.add_argument('--weight-path', type=str, default=None,help='weight path of the model')
    parser.add_argument('--layer-name', type=str, default='layer4hpa',
                        help='last convolutional layer name')
    parser.add_argument('--class-id', type=int, default=2047,
                        help='class id')
